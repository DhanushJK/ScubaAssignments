...
public class CucumberReporter {
	...
	public static void generateCucumberReport() {
		try {
			...
			CucumberResults[] results = objectMapper
					.readValue(new File(cucumberResultsDir + SEPARATOR + "cucumber.json"), CucumberResults[].class);
...
//                            Result beforeOutput = before.get(0).result;
							...
							for (Step step : steps) {
								...
								List<String> output = step.getOutput().stream().map(s -> s + "<br>")
								...
								if (testStatus) {
									...
									scenarioLogs.add(
											CustomReporter.appendStepFail(logs, name, count, scenarioName).toString());
									skipNextSteps = true;
								}
							}
